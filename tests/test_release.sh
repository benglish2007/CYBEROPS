...
set +e
verify_release_metadata 2.12.1 >/dev/null 2>&1
metadata_status=$?
set -e
...
notes="$(extract_release_notes 2.12.1)"
[[ "$notes" == *"Live Header Layout"* &&
    "$notes" != *"## 2.12"* ]]
...
build_release_artifact() { printf '%s/cyberops_2.12.1_all.deb' "$TEST_DIR"; }
...
publish_release 2.12.1 >/dev/null
record_result "publishes a validated release" "$?" 0
grep -Fq 'tag -a v2.12.1' "$MOCK_LOG"
record_result "creates an annotated version tag" "$?" 0
grep -Fq 'push origin v2.12.1' "$MOCK_LOG"
record_result "pushes only the version tag" "$?" 0
grep -Fq 'release create v2.12.1 --verify-tag' "$MOCK_LOG"
record_result "creates a release from the verified tag" "$?" 0
grep -Fq 'cyberops_2.12.1_all.deb#CYBEROPS v2.12.1 Debian package' "$MOCK_LOG"
...
set +e
publish_release 2.12.1 >/dev/null 2>&1
